...
import { LayoutDashboard, Store, LogOut, Zap, Bell, ChevronRight } from 'lucide-react'
...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex" style={{ backgroundColor: '#03030c' }}>
      {/* Sidebar */}
      <aside
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  {isActive && (
                    <ChevronRight className="w-3.5 h-3.5 ml-auto relative z-10 text-violet-400/60" />
                  )}
                </>
              )}
            </NavLink>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Topbar */}
        <header
          className="h-16 flex items-center gap-4 px-8 flex-shrink-0"
